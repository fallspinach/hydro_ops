...
def main() -> int:
    ...
    with lock_path.open("w") as lock:
        ...
        if any(any(marker in line for marker in stream_markers) for line in active):
        ...
        source_ids = list(
            dict.fromkeys(
                [*active_source_ids, *re.findall(r"Submitted batch job (\d+)", refresh.stdout)]
            )
        )
        ...
        command = [
            ...
            "slurm/continue_nwm_forcing_cycle.py",
        ]
