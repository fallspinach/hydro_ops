...
def write_precipitation_output(
    composite,
    ...
    *,
    ...
    mrms_quality_threshold: float,
    remap_grid_path: Path,
) -> Path:
    """Write one already-remapped precipitation composite in the operational schema."""
    valid_time = valid_time.replace(tzinfo=None)
    valid_time_text = valid_time.isoformat()
            ...
            output.setncatts(
                {
                    ...
                    "source_files": json.dumps(
                        {name: str(path) for name, path in source_files.items()}, sort_keys=True
                    ),
                    "quality_file": "none" if quality_file is None else str(quality_file),
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="hydro_ops_precipitation_", dir=scratch) as temporary:
        ...
        for product, source_path in items:
            ...
            with open_precipitation_candidate(
                source_path, product, valid_time=valid_time
            ) as normalized:
    ...
    return write_precipitation_output(
        composite,
        target_grid_path,
        output_path,
        valid_time=valid_time,
        source_files=candidate_paths,
