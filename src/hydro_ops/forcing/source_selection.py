...
def select_hourly_source(
    ...
    *,
    ...
    for index, product in enumerate(preference):
        ...
        path = source_path(product, roots[product], valid_time)
        if not path.is_file():
            rejected.append(f"{product}: missing {path}")
            continue
        try:
        ...
        if not inventory.valid:
            ...
            continue
        if inventory.valid_time != expected:
            rejected.append(
                f"{product}: time mismatch {inventory.valid_time!r}, expected {expected!r}"
            )
            continue
