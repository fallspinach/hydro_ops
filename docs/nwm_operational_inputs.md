...
The analysis namelist references `LAKEPARM_CONUS.nc`, but a matching NWM 3.1 file is absent from
the public `parm/domain` directory. A candidate NWM 2.1 file was rejected because its 5,783 lake
IDs differ from the 5,782-ID NWM 3.1 RouteLink/reservoir set. Operational scripts treat some
reservoir/lake parameters as dynamically managed inputs.

Until a validated NWM 3.1 lake file is available, the project uses a clearly labeled no-lake
configuration generated by `bin/create_nwm_no_lake_namelist.py`. It preserves the downloaded
operational namelist, sets `lake_option=0`, clears `route_lake_f`, and disables USGS, USACE, and
RFC reservoir assimilation. This is a scientifically different model configuration and must be
recorded in simulation provenance; it is not described as an operational NWM reproduction.
